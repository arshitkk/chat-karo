...
import { addMessage, setMessages } from "../utils/slices/chatSlice";
...
const MessageInput = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      dispatch(addMessage(response.data.savedMessage));

      // 🔥 Emit the message to the receiver via socket
      socket.emit("newMessage", response.data.savedMessage);
      // Clear form
      setText("");
      setImagePreview(null);
      if (fileInputRef.current) fileInputRef.current.value = "";
